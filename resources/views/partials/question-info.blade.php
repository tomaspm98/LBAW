    ...
    <div class="d-flex flex-column">
        ...
        <div id="action_buttons" class="text-center p-2">
            <form id="voteForm" action="{{ route('votes.voteQuestion', ['question_id' => $question->question_id]) }}" method="POST">
                ...
                class="btn p-2 rounded-top-5 vote-btn btn {{ $userVote && $userVote->upvote == 'up' ? 'btn-success' : 'btn-primary' }}">
                    <i class="bi bi-caret-up-fill"></i> <!--like-->
                </button>
                <p class="mt-3"><b id="voteCount">{{$question->vote_count}}</b></p>
                <button type="button" data-vote="down" 
                class="p-2 rounded-bottom-5 vote-btn btn {{ $userVote && $userVote->upvote == 'down' ? 'btn-danger' : 'btn-primary' }}">
            ...
            <ul class="dropdown-menu">
                ...
                <li>
                    <form action="{{ route('close.question', ['question_id' => $question->question_id]) }}" method="POST">
                        @csrf
                        <button class="dropdown-item text-danger" type="submit">Close Question</button>
                    </form>
                </li>
                @endif
