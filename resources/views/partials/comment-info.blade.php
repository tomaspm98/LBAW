...
<div class="comment_container w-100 bg-light mt-3 border-bottom rounded-2 p-1 d-flex position-relative" id="comment-{{$comment->comment_id}}">
    ...
    method="POST" class="text-center p-2"> 
        ...
        <button type="submit" data-vote="up" data-answer-id="{{$comment->answer_id}}" data-comment-id="{{$comment->comment_id}}"  
        class="btn vote-btn-comment p-2 rounded-top-5 {{ $userVote && $userVote->upvote == 'up' ? 'btn-success' : 'btn-primary' }}">
            <i class="bi bi-caret-up-fill"></i> <!--like-->
        </button>
        <p class="mt-3"><b id ="voteCountComment{{ $comment->comment_id }}">{{$comment->vote_count}}</b></p>
        <button type="submit" data-vote="down" data-answer-id="{{$comment->answer_id}}" data-comment-id="{{$comment->comment_id}}"
        class="btn vote-btn-comment p-2 rounded-bottom-5 {{ $userVote && $userVote->upvote == 'down' ? 'btn-danger' : 'btn-primary' }}">
